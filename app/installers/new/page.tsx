...
export default function NewInstallerPage() {
  ...
  }, [trainingCenter, settings, generateInstallerCode]);
  ...
  const checkCNIC = useCallback(async (cnicValue: string) => {
    ...
    try {
      const response = await fetch(`/api/installers?search=${cnicValue}`);
      const data = await response.json();

      if (data.success && data.data.installers.length > 0) {
        const existing = data.data.installers.find(
          (i: InstallerResponse) => i.cnic === cnicValue
        );
        if (existing) {
  ...
  // Debounce referrer code validation
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      <div className="max-w-4xl mx-auto">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Complete all steps to register a new installer
            </CardDescription>
          </CardHeader>
          <CardContent>
            {/* Steps Indicator */}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      {!cnicValidating && cnicChecked && cnic.length === 13 && (
                        <div className="absolute right-3 top-1/2 -translate-y-1/2">
                          <CheckCircle className="h-4 w-4 text-green-600" />
                        </div>
                      )}
                    </div>
                    {cnicChecked && !cnicValidating && (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={setTrainingCenter}
                    >
                      ...
                      <SelectContent>
                        {TRAINING_CENTER.map((tc) => (
                          <SelectItem key={tc.city} value={tc.city}>
                            {tc.city}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    ...
                    <div className="space-y-2">
                      ...
                      {installerCode && !settings?.allowInstallerCodeEdit && (
                        <p className="text-xs text-green-600 dark:text-green-500 flex items-center">
                          <Check className="h-3 w-3 mr-1" /> Code generated
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {referrerData && !referrerValidating && (
                      <p className="text-sm text-green-600 dark:text-green-500 flex items-center">
                        <Check className="h-4 w-4 mr-1" /> Referrer:{" "}
                        {referrerData.fullName} ({referrerData.installerCode})
                      </p>
                    )}
