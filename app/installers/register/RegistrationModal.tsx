...
export function RegistrationModal({
  ...
  // Paused while PIN delivery needs attention (blocked window or failed send).
  useEffect(() => {
    if (status !== "success" || !open) return;
    const blocked = deliveryMethod === "blocked" || (deliveryMethod !== "free-form" && whatsappFailed && !pinResent);
    if (blocked) return;
  ...
  }, [countdown, status, open, onRedirect, deliveryMethod, whatsappFailed, pinResent]);
              ...
              <div className="space-y-4 text-center">
                ...
                {deliveryMethod === "free-form" && (
                  <div className="bg-success/10 border border-success/30 rounded-2xl p-4 space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Free-form message delivered within the 24-hour window
                    </p>
                  </div>
                )}
                ...
                {deliveryMethod === "blocked" && (
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 space-y-3 text-left">
                    ...
                    <p className="text-xs text-muted-foreground">
                      The installer needs to send you a WhatsApp message first to reopen the 24-hour window. Then click &quot;Resend PIN&quot; to deliver credentials for free.
                    </p>
                    {whatsappMessage && (
                ...
                {/* Legacy fallback: auto-send disabled (no deliveryMethod set) */}
                {deliveryMethod !== "free-form" && deliveryMethod !== "blocked" && whatsappFailed && !pinResent && (
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 space-y-3 text-left">
                    <p className="text-sm font-medium text-amber-600 dark:text-amber-400">
                      {whatsappMessage
                ...
                {!(deliveryMethod === "blocked" || (deliveryMethod !== "free-form" && whatsappFailed && !pinResent)) && (
