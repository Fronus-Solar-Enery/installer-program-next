...
export async function GET(request: NextRequest) {
  try {
    ...
    const referrerPayments = rewards
      .filter((reward) => reward.referrer && reward.referrerRewardAmount && reward.referrerRewardAmount > 0)
      .map((reward) => {
    ...
    XLSX.utils.book_append_sheet(workbook, installerSheet, 'Installer Payments');

    if (referrerPayments.length > 0) {
      const referrerSheet = XLSX.utils.json_to_sheet(referrerPayments);
      XLSX.utils.book_append_sheet(workbook, referrerSheet, 'Referrer Payments');
    }
