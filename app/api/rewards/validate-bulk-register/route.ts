  ...
  async (request: NextRequest, context: RouteContext, session: AuthSession) => {
    try {
      ...
      if (!Array.isArray(rewards) || rewards.length === 0) {
        return ApiResponse.badRequest("Invalid rewards data");
      }
      ...
      const validatedRewards = rewards.map(
        (reward: RewardInput, index: number) => {
          ...
          const teamMemberEmail = reward.teamMemberEmail
            ?.toString()
            .toLowerCase();
          ...
          if (teamMemberEmail && !teamMemberEmails.has(teamMemberEmail)) {
            newIssues.push(
              `Team member email "${reward.teamMemberEmail}" does not exist in system`
            );
          }
          ...
          if (!installerCode) {
            ...
            newIssues.push(
              `Installer code "${reward.installerCode}" does not exist`
            );
          } else {
            ...
            if (
              reward.installerName &&
              installerInfo?.fullName !== reward.installerName
            ) {
              newIssues.push(
            ...
            if (installerInfo?.referrerCode) {
              ...
              if (!reward.referrerRewardAmount) {
                newIssues.push(
                  "Referrer reward amount is required for this installer (has referrer in system)"
                );
              }
              ...
              if (!reward.referrerTransactionId) {
                newIssues.push(
                  "Referrer transaction ID is required for this installer (has referrer in system)"
                );
              }
            }
          }
          ...
          if (!serialNumber) {
            ...
            newIssues.push(
              `Serial number "${reward.serialNumber}" already exists in database`
            );
          }
